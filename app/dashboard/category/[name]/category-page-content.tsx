...
export const CategoryPageContent = ({
  ...
  // http://localhost:3000/dashbaord/category/sale?page=5&limit=10
  ...
  const { data, isFetching } = trpc.category.getEventsByCategoryName.useQuery(
    {
      name: category.name,
  ...
  const NumericFieldSumCards = () => {
    if (Object.entries(numericFieldSums).length === 0) return null;
      ...
      return (
        <Card key={field}>
          <div className="flex flex-row items-center justify-between space-y-0 pb-2">
            <p className="text-sm/6 font-medium">{capitalize(field)}</p>
